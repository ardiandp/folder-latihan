...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE bpjs_mandiri SET nik=%s, no_akun=%s, nama=%s, no_va=%s, hub_keluarga=%s, terdaftar=%s, aktif=%s, parent=%s, karyawan=%s, kelas=%s, biaya=%s WHERE idbpjs=%s",
...
mysql_select_db($database_local, $local);
$query_edit = sprintf("SELECT * FROM bpjs_mandiri WHERE idbpjs = %s", GetSQLValueString($colname_edit, "int"));
$edit = mysql_query($query_edit, $local) or die(mysql_error());
$row_edit = mysql_fetch_assoc($edit);
$totalRows_edit = mysql_num_rows($edit);

mysql_select_db($database_local, $local);
$query_bpjs = "SELECT * FROM bpjs_mandiri";
$bpjs = mysql_query($query_bpjs, $local) or die(mysql_error());
$row_bpjs = mysql_fetch_assoc($bpjs);
$totalRows_bpjs = mysql_num_rows($bpjs);
?>
...
do {  
?>
        <option value="<?php echo $row_bpjs['nik']?>" <?php if (!(strcmp($row_bpjs['nik'], htmlentities($row_edit['parent'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>><?php echo $row_bpjs['nama']?></option>
        <?php
} while ($row_bpjs = mysql_fetch_assoc($bpjs));
?>
      </select></td>
    </tr>
    <tr> </tr>
